...
Ths problem is closely related to the Dutch Flag problem posed by Djikstra \ref{djikstraBook}, where a number of colored marbles need to be rearranged from a random order to a final, known configuration.  The correct solutions posed by Djikstra and Meyer were analyzed and found non-optimal by McMaster \ref{mcMaster78}.  A generalization to any number of colors was proposed and solved optimally \ref{optDutchFlag} using a scanning algorithm with insights that helped us form ours.  This generalization was later named the American Flag problem and shown to have applications for speeding up Radix sorting \ref{mcIlroy93}.

Another related problem is found in kidney exchanges and other market clearinghouses where a number of patient-donor pairs that aren't compatible require trades with other patient-donor pairs for compatible organs.  Market clearinghouses work like our 1D airplanes with large families that only need to seat together in pairs.  The problem is solved by finding a cycle cover of a graph, which was proven NP complete \ref{abrahamBlumSandholm07}.  Its impact on saving lives was both simulated \ref{kidney-roth04} and implemented \ref{abrahamBlumSandholm07}.  it was also proven that at allowing only 4 pairs of in any exchange chain is sufficient to guarantee an optimal cycle cover \ref{Roth_Kidney3way}.

Cycle covers of graphs also find an application in genetics.  This NP complete family of problems \ref{sankoffBlanchette97}, \ref{caprara97}, \ref{yu07} seeks the series of changes in DNA that lead from the DNA sequence of a common ancestor to two (or more) descendents.  Because the mutations and exchanges in DNA over time are functionally identical to swapping passengers, we can reformulate this as a constained seating rearrangement with known starting and ending poisitions.
...
\subsection{Optimality of Sweep Algorithm}
