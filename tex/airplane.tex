...
\subsubsection{Performing the Swap Sweep}
...
\subsubsection{Win-Win Exchanges, Completed Couples and N-Chains}

We make a quick note here on win-win exchanges.  A win-win exchange is of the form:
...
\subsubsection{Optimality of Sweep Algorithm}
...
In its current incarnation, our algorithm ignores all $N$-chains.  Since each $N$-chain requires $N-1$ swaps already, failing to optimally align our couple seats will cost us at most $\frac{N}{N-1}$ times the optimal number of exchanges.  That is, our current algorithm is a 2 approximation.4
